...
class Embedding_block_q(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        if self.apply_charge_doping:
            ...
            self.mlp_node = denseRegression(in_features=self.num_node_attr_feas, out_features=self.num_node_attr_feas,
                                            bias=True, use_batch_norm=False, n_h=2)
            self.mlp_q = denseRegression(in_features=self.num_charge_attr_feas, out_features=self.num_node_attr_feas,
                                         bias=True, use_batch_norm=False, n_h=2)
    ...
    def forward(self, data: AtomicDataDict.Type):
        ...
        if self.apply_charge_doping:
            q_num = data.doping_charge.long() + 9
