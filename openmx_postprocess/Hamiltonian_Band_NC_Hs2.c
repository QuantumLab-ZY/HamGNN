/**********************************************************************
  Hamiltonian_Band_NC_Hs2.c:

     Hamiltonian_Band_NC_Hs2.c is a subroutine to make an Hamiltonian matrix
  ...
  Log of Hamiltonian_Band_NC_Hs2.c:
			      ...
			      dcomplex *Hs2,  MPI_Comm mpi_commWD)
{
  ...
  for (i1=0; i1<na_rows; i1++){
    for (j1=0; j1<na_cols; j1++){
      ...
      Snd_Hs[id11][2*Num_Snd[id11]  ] = Hs11[j1*na_rows+i1].r;
      Snd_Hs[id11][2*Num_Snd[id11]+1] = Hs11[j1*na_rows+i1].i;
      Num_Snd[id11]++; 
      ...
      Snd_Hs[id22][2*Num_Snd[id22]  ] = Hs22[j1*na_rows+i1].r;
      Snd_Hs[id22][2*Num_Snd[id22]+1] = Hs22[j1*na_rows+i1].i;
      Num_Snd[id22]++; 
      ...
      Snd_Hs[id12][2*Num_Snd[id12]  ] = Hs12[j1*na_rows+i1].r;
      Snd_Hs[id12][2*Num_Snd[id12]+1] = Hs12[j1*na_rows+i1].i;
      Num_Snd[id12]++; 
      ...
      Snd_Hs[id21][2*Num_Snd[id21]  ] = Hs12[j1*na_rows+i1].r;
      Snd_Hs[id21][2*Num_Snd[id21]+1] =-Hs12[j1*na_rows+i1].i;
      Num_Snd[id21]++;
  ...
  for (ID=0; ID<numprocs; ID++){
    ...
    if (ID!=0){
      ...
      if (Num_Snd[IDS]!=0){
        ...
        /*  sending Snd_Hs */
        MPI_Isend(&Snd_Hs[IDS][0], Num_Snd[IDS]*2, MPI_DOUBLE, IDS, tag, mpi_commWD, &request);
      }

      if (Num_Rcv[IDR]!=0){

        /* receiving size */
        MPI_Recv(&Rcv_Hs[IDR][0], Num_Rcv[IDR]*2, MPI_DOUBLE, IDR, tag, mpi_commWD, &stat);
      }
