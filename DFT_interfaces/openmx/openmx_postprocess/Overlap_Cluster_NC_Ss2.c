...
void Overlap_Cluster_NC_Ss2( double *Ss, dcomplex *Ss2)
{
  ...
  for (ID=0; ID<numprocs; ID++){
    Snd_Ss[ID] = (double*)malloc(sizeof(double*)*(Num_Snd[ID]+1));
  }

  Rcv_Ss = (double**)malloc(sizeof(double*)*numprocs);
  for (ID=0; ID<numprocs; ID++){
    Rcv_Ss[ID] = (double*)malloc(sizeof(double*)*(Num_Rcv[ID]+1));
  }
  ...
  for (i1=0; i1<na_rows; i1++){
    for (j1=0; j1<na_cols; j1++){
      ...
      Snd_Ss[id11][Num_Snd[id11]] = Ss[j1*na_rows+i1];
      Num_Snd[id11]++; 
      ...
      Snd_Ss[id22][Num_Snd[id22]] = Ss[j1*na_rows+i1];
      Num_Snd[id22]++;
  ...
  for (ID=0; ID<numprocs; ID++){
    ...
    if (ID!=0){
      ...
      if (Num_Snd[IDS]!=0){
        ...
        /*  sending Snd_Ss */
        MPI_Isend(&Snd_Ss[IDS][0], Num_Snd[IDS], MPI_DOUBLE, IDS, tag, mpi_comm_level1, &request);
      }

      if (Num_Rcv[IDR]!=0){

        /* receiving size */
        MPI_Recv(&Rcv_Ss[IDR][0], Num_Rcv[IDR], MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
      }
  ...
  for (ID=0; ID<numprocs; ID++){
    for (k=0; k<Num_Rcv[ID]; k++){
      ...
      Ss2[(j2-1)*na_rows2+(i2-1)].r = Rcv_Ss[ID][k];
      Ss2[(j2-1)*na_rows2+(i2-1)].i = 0.0;
    }
  }
