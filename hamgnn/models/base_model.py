...
def find_matching_columns_of_A_in_B(A, B):
    ...
    # If A and B are very small, directly use the original method.
    if A.shape[1] * B.shape[1] * A.shape[0] < 10**6:
        A_rows = A.T.unsqueeze(1)
        B_rows = B.T.unsqueeze(0)
        matches = torch.all(A_rows == B_rows, dim=-1)
        matching_indices = matches.nonzero(as_tuple=True)[1]
        return matching_indices
    
    # Select some rows for quick comparison (the first row, a middle row and the last row).
    sample_indices = [0, A.shape[0]//2, -1]
    ...
    for i in range(A.shape[1]):
        # Check if the sample row matches
        col_a_sample = A_sample[:, i:i+1]
        sample_matches = torch.all(col_a_sample == B_sample, dim=0)
        potential_matches = torch.nonzero(sample_matches, as_tuple=True)[0]
        
        if potential_matches.numel() > 0:
            # For potential matches, check if the entire column matches.
            col_a = A[:, i:i+1]
            potential_B = B[:, potential_matches]
            equals = torch.all(col_a == potential_B, dim=0)
            true_matches = potential_matches[equals]
            
            if true_matches.numel() > 0:
                matching_indices.append(true_matches)
    
    if matching_indices:
        return torch.cat(matching_indices)
    else:
