...
double OutData(char *inputfile)
{
  ...
  if (specified_system != 0)
    out_coordinates_bulk();
  ...
  if (NHO_fileout == 1 || NBO_fileout == 1)
    out_Cluster_NBO();
...
void out_density()
{
  ...
  ****************************************************/
  ...
  if ((fp = fopen(fname, "w")) != NULL)
  {
    ...
    {
      ADensity_Grid_B[MN] = Density_Grid_B[0][MN] + Density_Grid_B[1][MN] - 2.0 * ADensity_Grid_B[MN];
    }
...
void out_Cluster_NBO() /* added by T.Ohwaki */
{
  ...
  ***********************************************/
  ...
  if (NBO_SmallCell_Switch == 1)
  {
    ...
    if (myid == Host_ID)
      printf("   $$$ Cell size: %10.6f %10.6f %10.6f \n", Leng_A, Leng_B, Leng_C);

    SCell_Origin[1] = tv[1][1] * NBO_SmallCellFrac[1][1] + tv[2][1] * NBO_SmallCellFrac[2][1] + tv[3][1] * NBO_SmallCellFrac[3][1] + Grid_Origin[1];
    SCell_Origin[2] = tv[1][2] * NBO_SmallCellFrac[1][1] + tv[2][2] * NBO_SmallCellFrac[2][1] + tv[3][2] * NBO_SmallCellFrac[3][1] + Grid_Origin[2];
    SCell_Origin[3] = tv[1][3] * NBO_SmallCellFrac[1][1] + tv[2][3] * NBO_SmallCellFrac[2][1] + tv[3][3] * NBO_SmallCellFrac[3][1] + Grid_Origin[3];
    ...
    SCell_Grid_Origin = Ngrid2 * Ngrid3 * SCell_Grid_A1 + Ngrid2 * SCell_Grid_B1 + SCell_Grid_C1;
    ...
    for (i = 0; i < SCell_GridN_A; i++)
    {
      for (j = 0; j < SCell_GridN_B; j++)
      {
        ...
        {
          GridList_L2SCell[tnum] = SCell_Grid_Origin + Ngrid2 * Ngrid3 * i + Ngrid3 * j + k;
          tnum++;
        }
      }
    }
    ...
    */
    ...
    for (Gc_AN = 1; Gc_AN <= atomnum; Gc_AN++)
    {
      ...
      if (myid == Host_ID)
        printf("   $$$ Gxyz[%d] : %10.6f %10.6f %10.6f\n", Gc_AN, tmp1, tmp2, tmp3);
  ...
  *************/
  ...
  if (NHO_fileout == 1)
  {
    ...
    if (myid == Host_ID)
      printf("<NBO> Construction of NHO-cube files \n\n");
  ...
  **************/
  ...
  if (NBO_fileout == 1)
  {
    ...
    if (myid == Host_ID)
      printf("<NBO> Construction of NBO-cube files \n\n");
      ...
      for (orbit = 0; orbit < Bulk_Num_HOMOs[kloop]; orbit++)
      {
        ...
        for (Mc_AN = 1; Mc_AN <= Matomnum; Mc_AN++)
        {
          ...
          for (Nc = 0; Nc < GridN_Atom[Gc_AN]; Nc++)
          {
            ...
            for (i = 0; i < NO0; i++)
            {
              ReCoef = co * HOMOs_Coef[kloop][spin][orbit][Gc_AN][i].r - si * HOMOs_Coef[kloop][spin][orbit][Gc_AN][i].i;
              ImCoef = co * HOMOs_Coef[kloop][spin][orbit][Gc_AN][i].i + si * HOMOs_Coef[kloop][spin][orbit][Gc_AN][i].r;
      ...
      for (orbit = 0; orbit < Bulk_Num_LUMOs[kloop]; orbit++)
      {
        ...
        for (Mc_AN = 1; Mc_AN <= Matomnum; Mc_AN++)
        {
          ...
          for (Nc = 0; Nc < GridN_Atom[Gc_AN]; Nc++)
          {
            ...
            for (i = 0; i < NO0; i++)
            {
              ReCoef = co * LUMOs_Coef[kloop][spin][orbit][Gc_AN][i].r - si * LUMOs_Coef[kloop][spin][orbit][Gc_AN][i].i;
              ImCoef = co * LUMOs_Coef[kloop][spin][orbit][Gc_AN][i].i + si * LUMOs_Coef[kloop][spin][orbit][Gc_AN][i].r;
              RMO_Grid_tmp[GN] += ReCoef * Orbs_Grid[Mc_AN][Nc][i]; /* AITUNE */
              IMO_Grid_tmp[GN] += ImCoef * Orbs_Grid[Mc_AN][Nc][i]; /* AITUNE */
            }
          }
