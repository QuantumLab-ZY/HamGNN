/**********************************************************************
  Cluster_DFT_OptBasis.c:

   Cluster_DFT_OptBasis.c is a subroutine to perform cluster calculations
   combined with simultaneous optimization of basis orbitals. 

  Log of Cluster_DFT_OptBasis.c:

     12/Aug./2017  Released by T.Ozaki
...
#pragma omp parallel shared(i1,n,spin,C,S,H,is2,ie2,myid1) private(OMPID,Nthrds,Nprocs,j1,sum0,sum1,sum2,sum3,l)
  ...
  ***********************************************/
  ...
  for (k=1; k<=MaxN; k++){
    ...
    sum = 0.0; 
    for (i=0; i<7; i++){
      sum += C[spin][k][124+i]*C[spin][k][124+i];
    }
    if (myid0==0) printf("ABC k=%2d sum=%15.12f\n",k,sum);
  }
  */
